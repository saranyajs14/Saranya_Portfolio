...
export default function Navbar() {
  ...
  if (!mounted) {
    return (
      <header className="sticky top-0 z-50 glass-dark backdrop-blur-sm shadow-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              <div className="w-10 h-10 rounded-full bg-slate-700 border-2 border-slate-500 flex items-center justify-center text-white font-bold text-xl">
                <span>JS</span>
              </div>
            </div>
            <nav className="hidden md:flex space-x-8">
              {navItems.map((item) => (
                <div key={item.id} className="px-4 py-2 text-slate-300">
                  {item.label}
                </div>
              ))}
            </nav>
